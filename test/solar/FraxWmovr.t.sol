...
contract FraxWmovrTest is BaseTest {
    struct Contracts {
    ...
    struct Paths {
        ...
        address[] fraxToMovrRoute;
        address[] movrToFraxRoute;
        address[] fraxToSolarRoute;
        address[] solarToFraxRoute;
        address[] fraxToUsdcRoute;
        address[] empty;
    ...
    function setUp() public {
        ...
        contracts.solarPair = ISolarPair(FRAX_MOVR_LP);
        ...
        paths.outputToLp0Route = [SOLAR, WMOVR, FRAX];
        paths.outputToLp1Route = [SOLAR, WMOVR];

        paths.fraxToMovrRoute = [FRAX, WMOVR];
        paths.movrToFraxRoute = [WMOVR, FRAX];
        paths.fraxToSolarRoute = [FRAX, WMOVR, SOLAR];
        paths.solarToFraxRoute = [SOLAR, WMOVR, FRAX];
        paths.fraxToUsdcRoute = [FRAX, WMOVR, USDC];
        ...
        underlying.asset = ERC20(FRAX_MOVR_LP);
        underlying.name = "FRAX-MOVR LP";
        underlying.symbol = "FRAX-MOVR";
        underlying.poolId = 37;
    ...
    function testRouterWarpInFromERC20(uint96 _amount) public {
        ...
        startHoax(user1);
        tip(FRAX, user1, _amount);
        IERC20(FRAX).approve(address(contracts.bayRouter), type(uint256).max);
        uint256 shares = contracts.bayRouter.warpIn(
            contracts.vault,
            IERC20(FRAX),
            paths.empty,
            paths.fraxToMovrRoute,
            _amount,
            5,
    ...
    function testRouterWarpOutToERC20(uint96 _amount) public {
        ...
        assertGt(IERC20(FRAX).balanceOf(address(user1)), 0);
    }
