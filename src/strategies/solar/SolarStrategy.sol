...
contract SolarStrategy is BayStrategy {
    ...
    function addLiquidity() internal returns (uint256 liquidityAdded) {
        uint256 outputBal = output.balanceOf(address(this));

        output.approve(address(warpIn), 0);
        output.approve(address(warpIn), outputBal);
        uint256 token0Amount = Utils.getAmountsOut(
            router,
            outputBal / 2,
            outputToLp0Route
        );
        uint256 token1Amount = Utils.getAmountsOut(
            router,
            outputBal / 2,
            outputToLp1Route
        );
        ...
        // console.log("beforeaddliq", outputBal);
    ...
    function _harvest(address harvestRewardRecipient) internal {
        ...
        if (outputBal > 0) {
            ...
            ISolarDistributorV2(chef).deposit(poolId, liquidityAdded);
