...
import {BayChef} from "../BayChef.sol";
import {IComplexRewarderYB} from "./IComplexRewarder.sol";
import {BoringERC20} from "../libraries/BoringERC20.sol";
import {IBoringERC20} from "../libraries/IBoringERC20.sol";

contract ComplexRewarderPerSec is IComplexRewarderYB, Ownable, ReentrancyGuard {
    using BoringERC20 for IBoringERC20;

    IBoringERC20 public immutable override rewardToken;
    BayChef public immutable chef;
    bool public immutable isNative;

    /// @notice Info of each BayChef user.
    /// `amount` bayLP token amount the user has provided.
    ...
    /// @notice Info of each BayChef pool.
    /// `accTokenPerShare` Amount of REWARD each bayLP token is worth.
    /// `startTimestamp` The start timestamp of rewards.
    /// `lastRewardTimestamp` The last timestamp REWARD was rewarded to the pool.
    ...
    modifier onlyBayChef() {
        require(
            msg.sender == address(chef),
            "onlyBayChef: only BayChef can call this function"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            Address.isContract(address(_chef)),
            "constructor: BayChef must be a valid contract"
        );
    ...
    function _updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        uint256 lpSupply = chef.poolTotalBayLp(pid);
    ...
    /// @notice Function called by BayChef whenever staker claims BAY harvest. Allows staker to also receive a 2nd reward token.
    ...
    function onBayReward(
        ...
        address _user,
        uint256 _amount
    ) external override onlyBayChef nonReentrant {
    ...
    function _pendingTokens(
        ...
        uint256 lpSupply = chef.poolTotalBayLp(_pid);
    ...
    /// @notice Withdraw reward. EMERGENCY ONLY.
    function emergencyWithdraw(uint256 _amount, address _beneficiary)
        external
        onlyOwner
        nonReentrant
    {
